...
static void reduce_once(unit *x)
{
    unit t;
    if (!uint_sub3(&t, x, &p))
        *x = t;
...
{
    uint_add3((unit *) x, (unit *) y, (unit *) z);
    reduce_once((unit *) x);
}
...
{
    if (uint_sub3((unit *) x, (unit *) y, (unit *) z))
        uint_add3((unit *) x, (unit *) x, &p);
}
...
void fp_enc(fp *x, unit const *y)
{
    fp_mul3(x, (fp *) y, &r_squared_mod_p);
}

void fp_dec(unit *x, fp const *y)
{
...
static void fp_pow(fp *x, unit const *e)
{
    fp y = *x;
    *x = fp_1;
    for (size_t k = 0; k < LIMBS; ++k) {
