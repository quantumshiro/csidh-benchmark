...
static bool validate_rec(proj *P, proj const *A, size_t lower, size_t upper, unit *order, bool *is_supersingular)
{
    assert(lower < upper);
...
bool validate(public_key const *in)
{
    ...
    {
        unit dummy;
        if (!uint_sub3(&dummy, (unit *) &in->A, &p)) /* returns borrow */
            /* A >= p */
            return false;
